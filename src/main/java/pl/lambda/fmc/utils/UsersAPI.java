...
public class UsersAPI
{
    ...
    public boolean create(String uuid, String name)
    {
        ...
        config.set("users." + uuid + ".money", 0);
        config.set("users." + uuid + ".isSiteDirector", false);
        save();
        return true;
    }
    ...
    public int getMoney(String uuid)
    {
        ...
        int discordid = config.getInt("users." + uuid + ".money");
        return discordid;
    }
    ...
    public void setMoney(String uuid, int money)
    {
        reload();
        if(!accountExist(uuid)) return;
        config.set("users." + uuid + ".money", money);
        save();
    }
    ...
    public void setSiteDirector(String uuid, boolean isSiteDirector)
    {
        reload();
        if(!accountExist(uuid)) return;
        config.set("users." + uuid + ".isSiteDirector", isSiteDirector);
        save();
    }

    public boolean getSiteDirector(String uuid)
    {
        reload();
        if(!accountExist(uuid)) return false;
        return config.getBoolean("users." + uuid + ".isSiteDirector");
    }
